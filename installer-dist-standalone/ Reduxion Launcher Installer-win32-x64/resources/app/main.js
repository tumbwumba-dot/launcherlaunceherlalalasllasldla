...
function createMainWindow() {
   try {
     ...
     mainWindow = new BrowserWindow({
       width: isInstaller ? 1000 : 1200,
       height: isInstaller ? 700 : 800,
       center: true,
       webPreferences: {
...
ipcMain.on('browse-gta-path', async (event) => {
  ...
  if (!result.canceled && result.filePaths.length > 0) {
    ...
    if (fs.existsSync(gtaExePath)) {
      ...
      dialog.showErrorBox('Ошибка', 'В выбранной папке не найден файл GTA5.exe');
    }
  }
});
      ...
      res.on('end', () => {
        try {
          ...
          if (versionInfo.version !== CURRENT_VERSION) {
            resolve({ hasUpdate: true, installerUrl: versionInfo.installerUrl });
          } else {
     ...
     mainWindow.webContents.on('did-finish-load', async () => {
       ...
       if (updateInfo.hasUpdate) {
         ...
         const response = await dialog.showMessageBox(mainWindow, {
           ...
           detail: 'Хотите установить обновление сейчас?',
         ...
         if (response.response === 0) {
           ...
           try {
             ...
             dialog.showErrorBox('Ошибка обновления', 'Не удалось скачать обновление: ' + error.message);
           }
         }
       }
